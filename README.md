...
B) Backend: B) Node.js/Express. It draws data from a MongoDB database and submits requests for scraping to a RabbitMQ message queue.  
C) Aforementioned MongoDB database  
D) Aforementioned RabbitMQ message queue.  
E) A celery process that retrieves messages from RabbitMQ and then spins up worker threads that use beautiful soup to scrape indeed. Stores results in Mongo. Will run a Tor process and direct requests through it if specified.   
